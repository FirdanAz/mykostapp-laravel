...
<div class="flex flex-col items-center justify-center py-20 text-center">
    ...
    <p class="text-slate-500 max-w-md mb-8">
        Anda belum mempunyai kos. Setup data kos Anda terlebih dahulu untuk mulai mengelola kamar dan penghuni.
    </p>
    <a href="{{ route('admin.kost.setup') }}"
       class="inline-flex items-center gap-2 px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-xl shadow-lg shadow-blue-600/20 transition-all">
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
        </svg>
        Setup Data Kos Sekarang
    </a>
</div>
@else
...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
    @php
    $cards = [
        ['label'=>'Total Kamar',      'value'=>$totalRooms,      'icon'=>'M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6', 'color'=>'blue',    'sub'=>"{$maintenanceRooms} sedang maintenance"],
...
{{-- ── Alert Cards ────────────────────────────────────────────── --}}
@if($unpaidInvoices > 0 || $activeComplaints > 0)
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="bg-white rounded-2xl border border-slate-200 p-6">
        <div class="mb-6">
            ...
            <p class="text-xs text-slate-400 mt-0.5">Saat ini</p>
        </div>
        <div style="height:180px"><canvas id="occupancyChart"></canvas></div>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
    ...
    <div class="bg-white rounded-2xl border border-slate-200">
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
            ...
            <a href="{{ route('admin.invoices.index') }}" class="text-xs text-blue-600 hover:text-blue-700 font-medium">Lihat semua →</a>
        </div>
        <div class="divide-y divide-slate-100">
            @forelse($recentInvoices as $invoice)
            <a href="{{ route('admin.invoices.show', $invoice) }}" class="flex items-center gap-3 px-6 py-3 hover:bg-slate-50 transition-colors">
                ...
                <div class="text-right flex-shrink-0">
                    ...
                    @php $sc=['unpaid'=>'bg-amber-100 text-amber-700','pending_verification'=>'bg-blue-100 text-blue-700','paid'=>'bg-green-100 text-green-700','overdue'=>'bg-orange-100 text-orange-700']; @endphp
    ...
    <div class="bg-white rounded-2xl border border-slate-200">
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100">
            ...
            <a href="{{ route('admin.complaints.index') }}" class="text-xs text-blue-600 hover:text-blue-700 font-medium">Lihat semua →</a>
        </div>
        <div class="divide-y divide-slate-100">
            @forelse($recentComplaints as $complaint)
            <a href="{{ route('admin.complaints.show', $complaint) }}" class="flex items-center gap-3 px-6 py-3 hover:bg-slate-50 transition-colors">
...
new Chart(document.getElementById('revenueChart'), {
    ...
    data: {
        ...
        datasets: [{ label:'Pendapatan', data: revenueData.map(d => d.total),
    ...
    options: {
        ...
        scales:{
            y:{grid:{color:'rgba(0,0,0,0.05)'},ticks:{callback:v=>'Rp '+Intl.NumberFormat('id-ID').format(v),font:{size:11}}},
...
new Chart(document.getElementById('occupancyChart'), {
    ...
    data:{
        ...
        datasets:[{data:[{{$occupiedRooms}},{{$availableRooms}},{{$maintenanceRooms}}],
            backgroundColor:['#3B82F6','#4ADE80','#FBBF24'],borderWidth:0,hoverOffset:4}]
    },
