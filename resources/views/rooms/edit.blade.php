...
@section('breadcrumb') <span class="mx-1">/</span> <a href="{{ route('admin.rooms.index') }}" class="hover:text-slate-600">Kamar</a> <span class="mx-1">/</span> Edit {{ $room->number }} @endsection
...
<form method="POST" action="{{ route('admin.rooms.update', $room) }}" enctype="multipart/form-data" class="space-y-6">
    ...
    <div class="flex gap-3">
        ...
        <a href="{{ route('admin.rooms.show', $room) }}" class="px-6 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold text-sm rounded-xl transition-colors">
            Batal
        </a>
    </div>
</form>
