...
<div class="bg-white rounded-2xl border border-slate-200 p-4">
    <form method="GET" class="flex flex-wrap gap-3">
        ...
        <a href="{{ route('rooms.index') }}" class="px-4 py-2 bg-slate-100 text-slate-600 text-sm font-medium rounded-xl hover:bg-slate-200 transition-colors">Reset</a>
        @endif
    </form>
</div>
...
<div class="bg-white rounded-2xl border border-slate-200 p-16 text-center">
    ...
    <a href="{{ route('rooms.create') }}" class="mt-4 inline-flex items-center gap-2 bg-blue-600 text-white text-sm font-medium px-4 py-2 rounded-xl hover:bg-blue-700 transition-colors">
    ...
    <div class="bg-white rounded-2xl border {{ $s['border'] }} overflow-hidden hover:shadow-lg transition-all duration-200 group">
        ...
        <div class="p-4">
            ...
            <div class="flex gap-2 pt-2 border-t border-slate-100">
                ...
                <form method="POST" action="{{ route('rooms.destroy', $room) }}" onsubmit="return confirm('Hapus kamar {{ $room->number }}?')">
                    @csrf @method('DELETE')
